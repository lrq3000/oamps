#!/bin/sh
echo "Starting OACL server OpenArena + ExcessivePlus on port 27960, screen oacl-xp and auto restart it at 4:02 AM everyday, and check if down each hour"

# Add to cron some warning messages that will be shown to the players minutes prior to restarting the servers
sh oamps.sh -n 0 -c oacl-edit-me -s oacl-xp -e "say ^1WARNING: Server will restart for maintenance in 10 minutes !" -a "52 3 * * *" -o oacl-launcher.log
sh oamps.sh -n 0 -c oacl-edit-me -s oacl-xp -e "say ^1WARNING: Server will restart for maintenance in 2 minutes !" -a "0 4 * * *" -o oacl-launcher.log
sh oamps.sh -n 0 -c oacl-edit-me -s oacl-xp -e "say ^1WARNING: Server will restart for maintenance NOW !" -a "1 4 * * *" -o oacl-launcher.log
...
sh oamps.sh -c oacl-edit-me -g excessiveplus -p 27960 -s oacl-xp -vm 0 -r -a "2 4 * * *" -o oacl-launcher.log
sh oamps.sh -c oacl-edit-me -g excessiveplus -p 27960 -s oacl-xp -vm 0 -a "@hourly" -o oacl-launcher.log
